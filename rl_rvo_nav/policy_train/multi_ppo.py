...
class multi_PPObuf:
    ...
    def get(self):
        ...
        obs_tensor_list = list(
            map(lambda o: torch.as_tensor(o, dtype=torch.float32), self.obs_buf))
...
class multi_ppo:
    def __init__(self, env, ac_policy, pi_lr=3e-4, vf_lr=1e-3, train_epoch=50, steps_per_epoch=600,
                 max_ep_len=300, gamma=0.99, lam=0.97, clip_ratio=0.2, train_pi_iters=100,
                 train_v_iters=100, target_kl=0.01, render=False, render_freq=20, con_train=False,
                 seed=7, save_freq=50, save_figure=False, save_path='test/', save_name='test',
                 load_fname=None, use_gpu=False, reset_mode=1, save_result=False, counter=0,
                 test_env=None, lr_decay_epoch=1000, max_update_num=10, mpi=False,
        ...
        self.buf_list = [multi_PPObuf(
            self.obs_dim, self.act_dim, steps_per_epoch, gamma, lam) for i in range(self.robot_num)]
        ...
        self.pt = post_train(test_env, reset_mode=reset_mode,
                             inf_print=False, render=False)
        if use_gpu:
    ...
    def training_loop(self):
        ...
        obs_list, ep_ret_list, ep_len_list = self.env.reset(
            mode=self.reset_mode), [0] * self.robot_num, [0] * self.robot_num
        ...
        for epoch in range(self.epoch + 1):
            ...
            for t in range(self.steps_per_epoch):
                ...
                if self.render and (epoch % self.render_freq == 0 or epoch == self.epoch):
                    self.env.render(save=self.save_figure,
                                    path=self.figure_save_path, i=t)
                ...
                for i in range(self.robot_num):
                    ...
                    cur_vel = np.squeeze(
                        self.env.ir_gym.robot_list[i].vel_omni)
                    abs_action = self.env.ir_gym.acceler * \
                        np.round(a_inc, 2) + cur_vel
                    # abs_action = 1.5*a_inc
                    abs_action = np.round(abs_action, 2)
                    abs_action_list.append(abs_action)

                next_obs_list, reward_list, done_list, info_list = self.env.step_ir(
                    abs_action_list, vel_type='omni')

                # save to buffer
                for i in range(self.robot_num):

                    self.buf_list[i].store(
                        obs_list[i], a_list[i], reward_list[i], v_list[i], logp_list[i])
                ...
                terminal = max(done_list) == True or max(
                    ep_len_list) > self.max_ep_len
            ...
            if (epoch % self.save_freq == 0) or (epoch == self.epoch):
                ...
                if self.save_result and epoch != 0:
                    # if self.save_result:
                    policy_model = self.save_path + \
                        self.save_name+'_'+str(epoch)+'.pt'
                    ...
                    thread = threading.Thread(target=self.pt.policy_test, args=(
                        'drl', policy_model, policy_name, result_path, '/results.txt'))
                    thread.start()
            ...
            print('The reward in this epoch: ', 'min',
                  min_ret, 'mean', mean, 'max', max_ret)
            ep_ret_list_mean = [[] for i in range(self.robot_num)]
            ...
            if self.mpi:
                if self.rank == 0:
                    ...
                    print('time cost in one epoch', time_cost, 'estimated remain time',
                          time_cost*(self.epoch-epoch)/3600, 'hours')
            else:
                time_cost = time.time()-start_time
                print('time cost in one epoch', time_cost, 'estimated remain time',
                      time_cost*(self.epoch-epoch)/3600, 'hours')
